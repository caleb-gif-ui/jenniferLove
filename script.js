...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ---------- CARROUSEL CIRCULAIRE (ROTATION 3D) ----------
  const totalSlides = slides.length;
  const angleStep = 360 / totalSlides; // 72° entre chaque image
  const radius = 320; // rayon du cercle en pixels (ajustable)
  let currentAngle = 0; // angle de rotation de l'anneau (en degrés)

  // Positionne chaque slide sur le cercle
  function positionSlides() {
    slides.forEach((slide, index) => {
      // L'angle de cette slide par rapport à l'avant (0°)
      const slideAngle = angleStep * index;
      // On applique une rotation Y et une translation Z pour la placer sur le cercle
      slide.style.transform = `rotateY(${slideAngle}deg) translateZ(${radius}px)`;
    });
  }

  // Met à jour la rotation de l'anneau pour amener la slide "currentIndex" à l'avant (angle 0)
  function rotateToIndex(index) {
    // L'angle cible : pour que la slide d'index donné soit face à nous, on tourne de -angleStep*index
  ...
  // Initialisation
  positionSlides();
  rotateToIndex(2); // commence sur Jennifer3 (index 2)
